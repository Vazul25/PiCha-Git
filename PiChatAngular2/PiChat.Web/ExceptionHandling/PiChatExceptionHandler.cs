...
namespace PiChat.Web.ExceptionHandling
{
    ...
    public class PiChatCustomErrorResult : IHttpActionResult
    {
        ...
        private HttpResponseMessage Execute()
        {
            ...
            {
                response = request.CreateErrorResponse(HttpStatusCode.Unauthorized, exception.Message);
            }
            else if (exception is PiChatGenericException)
            {
                response = request.CreateErrorResponse(HttpStatusCode.BadRequest, exception.Message);
            }
            else
            {
                response = request.CreateErrorResponse(HttpStatusCode.InternalServerError, "Unknown error occured");
            }
